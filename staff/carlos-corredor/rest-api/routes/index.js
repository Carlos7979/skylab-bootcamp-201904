...
const { UnauthorizedError, RequirementError } = require('../common/errors')
...
router.put('/users/', jsonParser, (req, res) => {
    handleErrors(() => {
        ...
        return logic.updateUser(data, token)
...
router.delete('/users/', jsonParser, (req, res) => {
    handleErrors(() => {
        ...
        if (!body) throw new RequirementError()
        ...
        return logic.deleteUser(token, password)
            .then(response => res.json(response))
    },
        res)
})
